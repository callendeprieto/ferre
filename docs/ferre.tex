...
This and any other linear tie can be implemented using the parameter NTIE 
(number of parameters tied) and the array INDTIE (indices of the parameters tied), 
together with the arrays TTIE0 and TTIE to specify the coefficients for the linear ties.
Thus, in general the values of a tied parameter P(INDTIE(i)) will be computed from other 
parameters P(j)


P(INDTIE(i)) = TTIE0(i) + $\sum_{\rm j=1,NDIM}$ TTIE(i,j) P(j).
