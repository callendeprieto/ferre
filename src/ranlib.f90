...
! Random samples from Probability Density Functions (PDF's), 
...
! Real uniform, Binomial, Negative Binomial, Multinomial, Poisson and Integer uniform
!
...
! for RANLIB to executed. The RNGLIB routines provide 32 virtual random number
! generators. Each generator can provide 1,048,576 blocks of numbers, and each 
! block is of length 1,073,741,824. Any generator can be set to the beginning or 
! end of the current block or to its starting value. Packaging is provided so 
! that if these capabilities are not needed, a single generator with period 
! 2.3 X 10^18 is seen. 
!
! Original FORTRAN77 version by Barry Brown and James Lovato. 
!
