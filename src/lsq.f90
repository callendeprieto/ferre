...
subroutine poly_fit( x, y, s, m, n, a, info )

!interface to the double-precision lsq module by Allan Miller
...
integer,intent(in)	 :: n  	    !order of the polynomial (number of free parameters -1)
real(dp),intent(in)	 :: x(m)    !abscissae
real(dp),intent(in)	 :: y(m)    !data
real(dp),intent(in)	 :: s(m)    !uncertainties in data y
real(dp),intent(out)	 :: a(n+1)  !coefficients
integer,intent(out)	 :: info    !error code

!locals	
real(dp)		 :: w(m)          !array of weights
real(dp)  		 :: xrow(0:n)   !tmp variable required by lsq
logical		   	 :: lindep(0:n) !variables required by lsq
logical		   	 :: fit_const     !
integer			 :: nreq          !
integer			 :: i,j           !dummy indices for loops


fit_const=.true.
call startup(n+1,fit_const)
a(:)=0.0_dp
do i=1,m
	w(i)=0.0_dp
	if (s(i) > 0._dp) w(i)=1._dp/s(i)**2
	xrow(0)=1._dp
	do j=1,n
		xrow(j)=x(i)**j
	enddo
...
enddo
call tolset
call sing(lindep,info)
nreq=n+1
call regcf(a,nreq,info)
