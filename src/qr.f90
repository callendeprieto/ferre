module qr
! Adapted from the qr_solve library (including routines from LINPACK and r8lib) from John Burkardt. 
! Much of the original code by Charles Lawson, Richard Hanson, David Kincaid, Fred Krogh.
! Added a polynomial fit interface (CAP)
...
subroutine polyfit( x, y, m, n, c)
  ...
  real(dp),intent(out)	 :: c(n+1)  !coefficients

  !locals	
  integer 		 :: i,j           !dummy indices for loops
  real (kind = 8)        :: a(m,n+1), b(m), xx(m), cc(n+1)
