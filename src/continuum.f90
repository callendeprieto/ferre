...
!       cont=-2 -- cubic splines fitting to the ratio x/o
!       cont=-1 -- polynomial fitting to the ratio x/o 
!	cont= 0 -- do nothing
		  ...
		  case (-2)
			...
			w2(1:nx)=1._dp
		        ...
		        if (nel2 <= 3) then
		          !give up 
		          write(*,*)'continuum: WARNING'
		          write(*,*) 'Too few points pass the rejectcont=',rejectcont,' filter', nel, 3, nel2
		          write(*,*) 'It is being ignored!'
		          nel2=nel
		          xaxis2(1:nel)=xaxis(1:nel)
		          x2(1:nel)=x(p1:p2)
		       ...
		       !fit splines
	               call spline_fit(xaxis2(1:nel2),x2(1:nel2)/ox2(1:nel2),nel2,ypp(1:nel2),error)

             	       !evaluate it
		       y(p1:p2)=0.0_dp
		       do i=1,nel
                           call spline_int(xaxis2(1:nel2),x2(1:nel2)/ox2(1:nel2),nel2,ypp(1:nel2),xaxis(p1+i-1),res,error)
                           y(p1+i-1)=res
		       enddo
	  	  case default
	    		write(*,*)'ferre: ERROR'
	    		write(*,*)'-- cont must be -2,-1,0,1,2,3'
